namespace Transpose.Plotly
{
    ...
    {
        public static Box<ITreemapProperty> legendgrouptitle(params Box<ILegendgrouptitleProperty>[] properties) => Interop.mkTreemapAttr("legendgrouptitle", Bindings.flattenProperties(properties));
        /// Sets the legend rank for this trace. Items and groups with smaller ranks are presented on top/left side while with `*reversed* `legend.traceorder` they are on bottom/right side. The default legendrank is 1000, so that you can use ranks less than 1000 to place certain items before all unranked items, and ranks greater than 1000 to go after all unranked items.
        public static Box<ITreemapProperty> legendrank(int val) => Interop.mkTreemapAttr("legendrank", val);
        /// Sets the legend rank for this trace. Items and groups with smaller ranks are presented on top/left side while with `*reversed* `legend.traceorder` they are on bottom/right side. The default legendrank is 1000, so that you can use ranks less than 1000 to place certain items before all unranked items, and ranks greater than 1000 to go after all unranked items.
        public static Box<ITreemapProperty> legendrank(float val) => Interop.mkTreemapAttr("legendrank", val);
        ...
        /// Assigns id labels to each datum. These ids for object constancy of data points during animation. Should be an array of strings, not numbers or any other type.
        public static Box<ITreemapProperty> ids(IEnumerable<string> values) => Interop.mkTreemapAttr("ids", values.ToArray());
        /// Assigns id labels to each datum. These ids for object constancy of data points during animation. Should be an array of strings, not numbers or any other type.
        ...
        /// Assigns extra data each datum. This may be useful when listening to hover, click and selection events. Note that, *scatter* traces also appends customdata items in the markers DOM elements
        public static Box<ITreemapProperty> customdata(IEnumerable<string> values) => Interop.mkTreemapAttr("customdata", values.ToArray());
        /// Assigns extra data each datum. This may be useful when listening to hover, click and selection events. Note that, *scatter* traces also appends customdata items in the markers DOM elements
        ...
        /// Sets the labels of each of the sectors.
        public static Box<ITreemapProperty> labels(IEnumerable<string> values) => Interop.mkTreemapAttr("labels", values.ToArray());
        /// Sets the labels of each of the sectors.
        ...
        /// Sets the parent sectors for each of the sectors. Empty string items '' are understood to reference the root node in the hierarchy. If `ids` is filled, `parents` items are understood to be \"ids\" themselves. When `ids` is not set, plotly attempts to find matching items in `labels`, but beware they must be unique.
        public static Box<ITreemapProperty> parents(IEnumerable<string> values) => Interop.mkTreemapAttr("parents", values.ToArray());
        /// Sets the parent sectors for each of the sectors. Empty string items '' are understood to reference the root node in the hierarchy. If `ids` is filled, `parents` items are understood to be \"ids\" themselves. When `ids` is not set, plotly attempts to find matching items in `labels`, but beware they must be unique.
        ...
        /// Sets the values associated with each of the sectors. Use with `branchvalues` to determine how the values are summed.
        public static Box<ITreemapProperty> values(IEnumerable<string> values) => Interop.mkTreemapAttr("values", values.ToArray());
        /// Sets the values associated with each of the sectors. Use with `branchvalues` to determine how the values are summed.
        ...
        public static Box<ITreemapProperty> sort(bool val) => Interop.mkTreemapAttr("sort", val);
        public static Box<ITreemapProperty> root(params Box<IRootProperty>[] properties) => Interop.mkTreemapAttr("root", Bindings.flattenProperties(properties));
